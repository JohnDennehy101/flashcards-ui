    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[631px] w-full lg:px-24 md:px-8 px-4 py-4">
            <div className="bg-neutral0 lg:w-2/3 h-fit lg:h-full flex items-center justify-center flex-col w-full border-1 border-neutral900 rounded-20 overflow-hidden">
                <FlashcardHeader />
                <FlashcardDisplay currentStep={1} totalSteps={5} question={"What does HTML stand for?"} answer={"HyperText Markup Language"} category={"Web Development"} showAnswer={showAnswer} onToggle={() => {setShowAnswer(!showAnswer);}} />
                <FlashcardFooter currentCard={1} totalCards={40} onPrevious={() => {}} onNext={() => {}} />
            </div>
            <div className="bg-neutral0 lg:w-1/3 w-full h-fit lg:h-full lg:items-stretch px-6 py-5 rounded-16 border-1 border-neutral900 overflow-hidden flex-col gap-4">
                <h2 className="text-preset2 font-poppins">Study Statistics</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4 flex-1 lg:flex lg:flex-col lg:justify-between mt-4">
