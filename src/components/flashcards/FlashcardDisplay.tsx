...
export function FlashcardDisplay({
  ...
  const displayCategory = useMemo(() => {
    ...
    const primaryLabel = matchedCategory || categories[0]

    return categories.length > 1
      ? `${primaryLabel} +${categories.length - 1}`
      : primaryLabel
  ...
  return (
    <div
      className={`bg-neutral0 ${containerHeight} w-full px-5 py-5 flex flex-col transition-all duration-300`}
    >
      <div
        onClick={onToggle}
        ...
        <div className="absolute top-6 w-full flex justify-center z-20">
          <div className="group relative">
            <Button
              ...
              className="text-preset6 font-poppins hover:translate-y-0 hover:shadow-none cursor-default bg-neutral0"
            />

            {categories.length > 1 && (
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden group-hover:flex flex-col gap-1 bg-neutral900 text-neutral0 p-2 rounded-8 z-50 min-w-[120px] pointer-events-none">
    ...
    ${type === "mcq" ? "justify-start pt-18" : "justify-center"}`}
          ...
          <div
            className={`absolute inset-0 flex flex-col items-center justify-center px-10 transition-all
        ...
        <div
          className={`absolute ${currentStep < totalSteps ? "bottom-6 w-18" : "bottom-4"} left-1/2 -translate-x-1/2 z-10`}
        >
        ...
        <div
          className={`absolute bottom-10 transition-all duration-300 ease-in-out ${showAnswer ? "left-6" : "left-10"}`}
        >
      ...
      >
        <div className="space-y-6 py-2">
          <div className="space-y-2">
            <h4 className="text-preset5 font-bold text-neutral900">
              Justification
            </h4>
