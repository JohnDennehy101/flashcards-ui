...
export function CardMenu({ onEdit, onDelete, isOpen, onToggle }: CardMenuProps) {
        ...
        <div className="relative border-l p-2.5 py-4 flex items-center">
            {isOpen && (
                <div className="absolute bottom-full right-2 mb-2 w-32 border bg-neutral0 rounded-lg shadow-sm z-10 overflow-hidden">
                    <Button text={"Edit"} onClick={() => handleAction(onEdit)} icon={<EditIcon />} iconPosition={"start"} variant={"menu"} className="border-b" />
                    <Button text={"Delete"} onClick={() => handleAction(onDelete)} icon={<DeleteIcon />} iconPosition={"start"} variant={"menu"} />
                </div>
            )}
